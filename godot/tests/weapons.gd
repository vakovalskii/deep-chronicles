...
func _run():
	...
	for row in 2:
		for col in 3:
			...
			var actor = Actor.new(); actor.kind = "self"; scene.add_child(actor); actor.setup(id, id + " · " + ["idle", "walk", "attack"][col])
			actor.position = Vector3((col - 1) * 3.6, 0, row * 4.0); actor.rotation.y = -0.3
			actor.apply_look({"w": 0xbbccd8, "staff": id == "mage", "gear": {}, "ench": 4})
			actor.set_process(false); actor.animator.play(["idle", "walk", "attack"][col]); actor.animator.seek([0.4, 0.2, 0.3][col], true); actor.animator.pause()
			actors.append(actor)
	...
	var camera = Camera3D.new(); scene.add_child(camera); camera.position = Vector3(-1, 6, -12); camera.look_at(Vector3(0, 1, 1.8)); camera.fov = 43; camera.current = true
	...
	print("WEAPON_TEST_RESULT checks=6 failures=", failures)
