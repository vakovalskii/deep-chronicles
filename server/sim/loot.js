...
export function createGroundLoot() {
  ...
  function spawn(position, reward, owner, ownerName, now = Date.now()) {
    const items = [];
    ...
    return items.map((item, i) => {
      ...
      const d = { id: randomUUID(), ...item, x: pos.x, y: pos.y, z: pos.z, owner, ownerName,
  ...
  function claim(id, actor, account, now = Date.now()) {
    ...
    if (now < d.protectedUntil && account !== d.owner) return { error: `Добыча пока принадлежит ${d.ownerName}.` };
    entries.delete(id);
    return { drop: d };
  }
  function snapshotFor(actor, view, account, now = Date.now()) {
    expire(now);
    return [...entries.values()].filter(d => flatDist(actor, d) < view).map(({ owner, ...d }) => ({ ...d, available: owner === account || now >= d.protectedUntil }));
  }
