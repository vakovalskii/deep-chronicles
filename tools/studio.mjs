...
const TYPES = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript', '.json': 'application/json', '.glb': 'model/gltf-binary', '.png': 'image/png' };
...
const readRig = () => (fs.existsSync(RIG) ? JSON.parse(fs.readFileSync(RIG, 'utf8')) : {});
...
http.createServer(async (req, res) => {
  ...
  if (url === '/api/list') return json(200, { models: list(MODELS, '.glb'), icons: list(ICONS, '.png').filter((i) => !i.startsWith('_')), rig: readRig() });
